...
/** Parsed session command with optional argument. */
export interface ParsedSessionCommand {
  command: '/compact' | '/clear' | '/resume';
  arg?: string;
}
 ...
 */
export function extractSessionCommand(content: string, triggerPattern: RegExp): ParsedSessionCommand | null {
  let text = content.trim();
  text = text.replace(triggerPattern, '').trim();
  if (text === '/compact') return { command: '/compact' };
  if (text === '/clear') return { command: '/clear' };
  const resumeMatch = text.match(/^\/resume\s+(\S+)$/);
  if (resumeMatch) return { command: '/resume', arg: resumeMatch[1] };
  return null;
}
...
export interface SessionCommandDeps {
  ...
  /** Set the session for the group (host-side session restore). */
...
export async function handleSessionCommand(opts: {
  ...
  const parsed = cmdMsg ? extractSessionCommand(cmdMsg.content, triggerPattern) : null;

  if (!parsed || !cmdMsg) return { handled: false };
  ...
  // /resume is handled entirely host-side — sets session to the given ID
  if (command === '/resume') {
    deps.closeStdin();
    deps.setSession(parsed.arg!);
    deps.advanceCursor(missedMessages[missedMessages.length - 1].timestamp);
    await deps.sendMessage(`Resumed session: ${parsed.arg}`);
    return { handled: true, success: true };
  }

  // /compact and /clear are forwarded to the container agent as SDK slash
  // commands. The SDK handles /compact (summarizes history) and /clear
  // (starts a fresh session — the new session ID comes back via
  // newSessionId in the container output and is tracked automatically).
  const cmdIndex = missedMessages.indexOf(cmdMsg);
  const preCommandMsgs = missedMessages.slice(0, cmdIndex);

  // Process pre-command messages so they're in the session context before
  // compaction. Skip for /clear since the session is being wiped anyway.
  if (command !== '/clear' && preCommandMsgs.length > 0) {
